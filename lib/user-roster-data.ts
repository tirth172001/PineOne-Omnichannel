...
export type RosterEntry = {
  ...
  role: string
  status: UserStatus
  /** Store this user is assigned to, for Manage stores' store-detail Users tab. Undefined for
   *  merchant-wide users with no single-store assignment. */
  storeId?: string
}
...
const SEED_ROSTER: RosterEntry[] = [
  ...
  { id: "role-1", name: "Karan Joshi", email: "karanjoshi77@gmail.com", addedOnDate: "23 Jun 2026", addedOnTime: "01:20 PM", scope: scopeForRoleName("Store Manager"), role: "Store Manager", status: "Active", storeId: SEED_STORE_IDS[0] },
  ...
  { id: "role-5", name: "Priya Singh", email: "priyasingh65@yahoo.com", addedOnDate: "21 Jun 2026", addedOnTime: "02:30 PM", scope: scopeForRoleName("Store Cashier"), role: "Store Cashier", status: "Active", storeId: SEED_STORE_IDS[0] },
  { id: "role-6", name: "Neha Verma", email: "nehaverma88@outlook.com", addedOnDate: "27 Jun 2026", addedOnTime: "04:30 PM", scope: scopeForRoleName("Admin"), role: "Admin", status: "Active", storeId: SEED_STORE_IDS[1] },
  { id: "role-7", name: "Aditi Sharma", email: "aditisharma99@hotmail.com", addedOnDate: "20 Jun 2026", addedOnTime: "11:15 AM", scope: "In-store", role: "Admin", status: "Invited", storeId: SEED_STORE_IDS[1] },
...
function buildGeneratedRosterEntry(index: number): RosterEntry {
  ...
  return {
    ...
    storeId: SEED_STORE_IDS[index % SEED_STORE_IDS.length],
  }
}
...
export function usersForStore(storeId: string, roster: RosterEntry[] = INITIAL_ROSTER) {
  return roster.filter((entry) => entry.storeId === storeId)
}

export function usersInvitedCount(storeId: string, roster: RosterEntry[] = INITIAL_ROSTER) {
  return roster.filter((entry) => entry.storeId === storeId && entry.status !== "Removed").length
}
