...
function CategoryRow({
  ...
  return (
    <button onClick={onClick} className={`intercom-panel-row ${selected ? "intercom-panel-row-active" : ""}`}>
      ...
      <div className="space-y-2">
        {category.products.map((product) => {
          ...
          return (
            <button
              key={product.id}
              className={`w-full rounded-lg p-4 text-left transition-colors ${selectedProductId === product.id ? "bg-secondary/70" : "bg-card/80 hover:bg-muted/50"}`}
              ...
              <div className="flex items-start gap-3">
                ...
                <div className="min-w-0 flex-1">
                  <div className="mb-1 flex items-center gap-2">
                    ...
                    {isEnabled && (
                      <Badge variant="outline" className="text-[9px] border-success/30 bg-success/10 text-success px-1.5 py-0">
                        <span className="h-1.5 w-1.5 rounded-full bg-success" /> Enabled
                      </Badge>
                    )}
                    {isComingSoon && <Badge variant="outline" className="text-[9px]">Coming soon</Badge>}
                  </div>
...
export function ProductsContent() {
  ...
  const leftContext = (
    <div className="h-full overflow-y-auto p-3">
      <p className="px-1 text-[10px] uppercase tracking-[0.14em] text-muted-foreground">Product Areas</p>
