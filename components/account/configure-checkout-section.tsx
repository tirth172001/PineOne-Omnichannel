...
import { ColorPickerPopover } from "@/components/account/checkout-color-picker"
...
function SettingsAccordionItem({
  ...
  return (
    <AccordionItem
      value={value}
      className="overflow-hidden rounded-lg border border-border bg-card px-5 shadow-xs"
    >
      <AccordionTrigger className="py-4 text-base font-semibold text-foreground hover:no-underline focus-visible:border-transparent focus-visible:ring-0">
        {title}
      </AccordionTrigger>
      <AccordionContent className="divide-y divide-border/60 pt-0 pb-0">{children}</AccordionContent>
    </AccordionItem>
  )
}
      ...
      <div className="flex h-full min-h-0 flex-col">
        <div className="min-h-0 flex-1 space-y-3 overflow-y-auto pr-1">
          <Accordion type="multiple" value={openSections} onValueChange={setOpenSections} className="space-y-3">
            <SettingsAccordionItem value="checkout-branding" title="Checkout branding">
              <SettingsRow
            ...
            <SettingsAccordionItem value="wallet-branding" title="Wallet branding">
            ...
            <SettingsAccordionItem value="express-checkout" title="Express checkout preferences">
              <TogglePreferenceRow
      ...
      <div className="flex h-full min-h-0 flex-col rounded-2xl bg-muted/40 p-6">
          ...
          <div className="flex items-center gap-2">
            {previewDevice === "desktop" ? (
              <div className="flex items-center gap-1 rounded-md border border-border/70 bg-muted p-1">
                <button
                  type="button"
                  onClick={() => setDesktopZoom((z) => Math.max(ZOOM_MIN, z - ZOOM_STEP))}
                ...
                <span className="w-9 text-center text-xs font-medium text-foreground">{desktopZoom}%</span>
                <button
                  type="button"
                  onClick={() => setDesktopZoom((z) => Math.min(ZOOM_MAX, z + ZOOM_STEP))}
        ...
        <div className="flex min-h-0 flex-1 items-start justify-center overflow-auto py-4">
          <div
            ...
            style={previewDevice === "desktop" ? { width: DESKTOP_PREVIEW_WIDTH, zoom: desktopZoom / 100 } : undefined}
          >
...
export function ConfigureCheckoutSection() {
      ...
      >
        <div className="border-b border-border/70 px-8 pt-6">
          <div className="flex flex-wrap items-center justify-between gap-3 pb-5">
            <h1 className="text-2xl font-bold leading-none tracking-[-0.01em] text-foreground">Configure checkout</h1>
          </div>
          <TabsList variant="line" className="h-9 w-fit gap-6 p-0">
            <TabsTrigger value="customisation" className="flex-none px-1 text-sm data-active:font-semibold after:bg-primary">
              Customisation
            </TabsTrigger>
            <TabsTrigger value="paymodes" className="flex-none px-1 text-sm data-active:font-semibold after:bg-primary">
              Paymodes
            </TabsTrigger>
          </TabsList>
        </div>
